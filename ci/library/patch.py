...
# PEP 3114
compat_next = (lambda gen: gen.next()) if not PY3K else lambda gen: gen.__next__()
...
def xisabs(filename):
    ...
    """
    if filename.startswith((b"/", b"\\")) or re.match(
        b"\\w:[\\\\/]",
        filename,
    ):  # Linux/Unix
        return True
...
class PatchSet:
    ...
    def parse(self, stream):
        ...
        hunkactual = {"linessrc": None, "linestgt": None}
    ...
    def _detect_type(self, p):
        ...
        if len(p.header) > 1:
            ...
            for idx in reversed(range(len(p.header))):
                if p.header[idx].startswith(b"diff --git"):
                    break
            if (
                p.header[idx].startswith(b"diff --git a/")
                and (
                    idx + 1 < len(p.header)
        ...
        if len(p.header) > 0:
            ...
            if DVCS and p.header[-1].startswith(b"diff --git a/"):
                if len(p.header) == 1 or p.header[0].startswith(
                    b"# HG changeset patch",
                ):  # native Git patch header len is 2
                    return HG
    ...
    def _normalize_filenames(self):
        ...
        for i, p in enumerate(self.items):
            ...
            if xisabs(p.source) or xisabs(p.target):
                ...
                self.warnings += 1
                if xisabs(p.source):
                    warning(f"stripping absolute path from source name '{p.source}'")
                    p.source = xstrip(p.source)
                if xisabs(p.target):
                    warning(f"stripping absolute path from target name '{p.target}'")
    ...
    def diffstat(self):
        ...
        for i, n in enumerate(names):
            ...
            # -- /calculating +- histogram --
            output += format % (tostr(n), str(insert[i] + delete[i]), hist)
    ...
    def apply(self, strip=0, root=None):
        ...
        if strip:
            ...
            try:
                ...
                errors += 1
                warning(f"error: strip parameter '{strip}' must be an integer")
                strip = 0

        # for fileno, filename in enumerate(self.source):
        for i, p in enumerate(self.items):
            if strip:
                debug(f"stripping {strip} leading component(s) from:")
                debug(f"   {p.source}")
                debug(f"   {p.target}")
            ...
            for lineno, line in enumerate(f2fp):
                ...
                if lineno + 1 < hunk.startsrc + len(hunkfind) - 1:
                    if line.rstrip(b"\r\n") == hunkfind[hunklineno]:
                        ...
                        info("  actual  : {}".format(line.rstrip(b"\r\n")))
            ...
            if validhunks < len(p.hunks):
                if self._match_file_hunks(filename, p.hunks):
                    warning(f"already patched  {filename}")
                else:
                    warning(f"source file is different - {filename}")
                    errors += 1
            if canpatch:
                ...
                if exists(backupname):
                    ...
                    if self.write_hunks(backupname, filename, p.hunks):
                        ...
                        errors += 1
                        warning(f"error patching file {filename}")
                        shutil.copy(filename, filename + ".invalid")
                        warning(f"invalid version is saved to {filename}" + ".invalid")
                        # todo: proper rejects
            ...
            for hline in h.text:
                ...
                if hline.startswith((b"-", b"\\")):
                    ...
                    if sum([bool(lineends[x]) for x in lineends]) == 1:
                        newline = next(x for x in lineends if lineends[x] != 0)
...
def main():
    ...
    opt = OptionParser(
        ...
        version=f"python-patch {__version__}",
    )
    opt.add_option(
        "-q",
        "--quiet",
    ...
    if readstdin:
        ...
        if (
            ...
            if not exists(patchfile) or not isfile(patchfile):
                sys.exit(f"patch file does not exist - {patchfile}")
